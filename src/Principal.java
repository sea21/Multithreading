...
public class Principal {
	...
	public static void main(String[] args) throws IOException {
		...
		createDocument(nroComprobantes);
		
	}
	
	public static Document createDocument(int nroComprobantes) throws IOException {
        ...
        for (int i=0; i<nroComprobantes; i++){
        	...
        	int emp = (int)Math.floor(Math.random()*(0-(codigosTerceros.size()+1))+(codigosTerceros.size()+1));
        	
        	int ter = (int)Math.floor(Math.random()*(0-(codigosTerceros.size()+1))+(codigosTerceros.size()+1));
            ...
            int cuenta1 = (int)Math.floor(Math.random()*
            		(0-(codCuentas.size()+1))+(codCuentas.size()+1));
            ...
            XMLWriter writer = new XMLWriter(
                new FileWriter( "salidas/comprobante"+ (i+1) + ".xml" )
            );
